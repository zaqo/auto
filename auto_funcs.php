...
function show_fuel_log_card($db_server,$id)
{
	$nav_bar_mobile= car_menu_maker($id);
	$content="";
	$content.=$nav_bar_mobile;	
		...
		//CARD HEADER
					...
					$content.= '		<div class="card-header">Топливный журнал</div>';
		...
		if ($tot_qty)
		{
			...
			$content.='<span>&nbsp&nbsp&nbsp&nbsp; ИТОГО: &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp'.number_format($tot_qty).' '.$mu_name.'</span>&nbsp&nbsp&nbsp&nbsp&nbsp <span>'.number_format($tot_price).' '.$cur_name.'</span>';
		}
		else
...
/* 
	...
	26/09/18
*/
function show_km_log_card($db_server,$id)
{
	$nav_bar_mobile= car_menu_maker($id);
	$content="";
	$content.=$nav_bar_mobile;	
		...
		//CARD HEADER
					...
					$content.= '<div class="card border-primary mb-3 ml-5 mt-5" style="max-width: 28rem;">';
					$content.= '		<div class="card-header">Пробег автомобиля</div>';
					$content.= '			<div class="card-body text-primary">';
		...
		{ 
				...
				if($isFirst)
				{
					//$content.= '	<li class="list-group-item"><small> <span>Дата</span><span>Пробег,'.$mu.'</span><span>   Итого, '.$mu.'</span></small></li>';
					$col_count.='<p>'.$counter.'</p>';
					$col_date.='<p>'.$trn_date_cl.'</p>';
					$col_mes.='<p ></p>';
		...
		//$content.= '<li class="list-group-item"><b>ИТОГО:</b> '.$tot_qty.' </li>';
				//$content.= '</ul>';
		$content.= '</div>';
		$content.= '</div>';
		$content.= '</div>';
